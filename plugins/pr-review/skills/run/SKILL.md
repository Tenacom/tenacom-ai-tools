...
Corollary on links: **every link to code is written one way — the VS Code-relative `](./path#<line>)` form** — in `###` headings and in prose alike (evidence inside a block, a pointer in a body paragraph). The fragment is the bare start line, `#52` never `#L52`, exactly as the heading link below; a line range, when there is one, rides in the link **text** (`foo.ts L52-L60`), never in the target. The `#<line>` fragment is **compulsory** — never link to a whole file, and never hand-write a GitHub permalink. `pr-finalize` rewrites every prose and body link to a permalink when it posts — the body and the block prose post to GitHub, where a relative link resolves to nothing — while the file on disk stays relative, so every link is navigable in the local checkout while you curate.
