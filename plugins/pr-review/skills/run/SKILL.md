...
Corollary on links: **every link to code is written one way — the VS Code-relative `](./path#<line>)` form** — in `###` headings and in prose alike (evidence inside a block, a pointer in a body paragraph). The fragment is the bare start line, `#52` never `#L52`, exactly as the heading link below; a line range, when there is one, rides in the link **text** (`foo.ts L52-L60`), never in the target. Never hand-write a GitHub permalink: `pr-finalize` rewrites every prose and body link to one when it posts — the body and the block prose post to GitHub, where a relative link resolves to nothing — while the file on disk stays relative, so every link is navigable in the local checkout while you curate.

The `#<line>` fragment is **compulsory in a `###` heading**, whose link is the finding's anchor. **In prose it is compulsory too, with one exception: a whole-file link, `](./path)` with no fragment, when the _file_ and not a line is what you are pointing at** — a module the PR adds, a config file that has to be edited alongside the change. Anything you point at _inside_ a file has a line, so link the line: "see [the parser](./src/parser.ts)" is the shape to avoid, because it makes the reader hunt for what you meant.
...
- Evidence pointing _elsewhere_ (the caller that breaks, the twin that survives, the rule text's source) goes inline in the prose as VS Code-relative links (`](./path#<line>)`), which `pr-finalize` converts to permalinks when it posts.
...
- **Caused by the change, defect outside it** (the loop body was optimized and the exit condition above it is now wrong; a parameter list changed and the docblock went stale): locate at the **causing hunk**, name the affected line in the prose with a relative link, and open causally — "here the route becomes …, so … at \<link\> now …". Lead-with-cause then arrives by structure, not by discipline.
- **The PR fixes one instance and an identical twin survives elsewhere**: locate at the **fixed instance** — "the same defect survives at \<link\>" — framed as consistency cleanup, not as a fault of this PR.
...
- Code links: never hand-write a GitHub permalink. Every link to code — in a `###` heading and in prose — is the VS Code-relative Markdown link `[<text>](./<path>#<start>)`: the target's path is repo-root-relative, its fragment the **bare** start line (`#52`, never `#L52` — GitHub's fragment grammar is not VS Code's, but the tool handles that), and the line range, when there is one, rides in the visible **text** (`user-cms-detail.component.ts L52-L60`). `pr-finalize` rewrites each prose and body link to a `blob/<head>` permalink at post time — pinned to the reviewed head, so the posted link is exact and survives the merge, and stays exact even if the PR head moves before the review is posted. A `###` heading's link always carries the fragment; in prose you may drop it, `](./path)`, when the whole file is the referent (see the links corollary). A fragment that is not a bare line number — `#L52`, a section anchor — is refused by name: it is a broken link, not a whole-file one.
...
The route becomes `user-cms/:id`, so the id now travels in the _path_ (the `/123` segment of the URL), no longer in the _query string_ (a would-be `?id=123`). The component still reads it from the query ([user-cms-detail.component.ts L52](./projects/example/src/app/user-cms/user-cms-detail.component.ts#52)): on `/user-cms/123` the query holds no `id`, so `id` stays `0` and the "missing id" guard fires.
