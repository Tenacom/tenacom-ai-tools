...
- **Report in English, register-neutral.** Whatever language the PR is written in, your report is plain English — the main agent renders the PR's language and the selected register (see Register) when it assembles `REVIEW.md`. Do not translate, localise, or soften your findings; state each one plainly so the assembly step has clean material to render.
...
- **Every block spends its prose on the defect.** What is wrong, what triggers it, what follows — the marquee finding, the longest here, gives every sentence to the divergence and its consequence; the code itself is left to the anchor, where the reader already has it.
