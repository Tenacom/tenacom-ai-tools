...
A pending review would otherwise block posting (GitHub allows one pending review per user per PR), so `pr-finalize` detects yours and, instead of posting a separate review, **adds this review's findings to yours and submits it as one review**. Your own comments are never rebuilt or deleted — whatever you wrote rides along exactly as-is, including replies to existing threads and file-level comments (which earlier versions had to refuse). Your draft summary text, if any, becomes the review body. A pending comment counts as a finding: the submitted review requests changes even if no checkbox is ticked. The recap shows what will be added, and the fetched review is saved to `.pr-review-run/pending-import.json` for reference. `pr-finalize --dry-run` shows what would be added and touches nothing on GitHub.

**One edge to know about.** Your findings are added to your pending review pinned to the reviewed commit. When the pending review happens to sit at a different commit than the one you reviewed, that pin can only be single-line — so a checked finding spanning a **range of lines** cannot be added there without dropping its range (which would break a multi-line `suggestion` block). `pr-finalize` refuses rather than mangle it, names the finding, and holds the whole post back. The fix is either to submit or discard your pending review on github.com and re-run (your findings then post as one review with ranges intact), or to narrow that finding to a single line. In the common case — no pending review, or one at the same commit you reviewed — ranges post normally.
